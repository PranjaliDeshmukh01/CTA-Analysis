...
def get_snow_vs_total_rides():
    ...
    joined_pd = df.select("date", "SNOW (Inches)", "total_rides").orderBy("SNOW (Inches)").toPandas()
    ...
    # plt.yscale("log")
    # sns.scatterplot(x="SNOW (Inches)", y="total_rides", data=joined_pd, marker="o")
    # plt.title("Snow vs. Total Rides")
    # plt.xlabel("Snow (Inches)")
    # plt.ylabel("Total Rides")
    # plt.tight_layout()

    plt.figure(figsize=(12, 6))
    plt.hexbin(joined_pd["SNOW (Inches)"], joined_pd["total_rides"], gridsize=20, cmap='viridis', mincnt=1, reduce_C_function='mean')

    # Add colorbar for reference
    plt.colorbar(label="Average Rides in Bin")

    plt.title("Hexbin Plot of Snow vs. Total Rides")
    plt.xlabel("Snow (Inches)")
    plt.ylabel("Total Rides")
    plt.tight_layout()
    plt.show()
